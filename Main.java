...
public class Main {
   ...
   public static User registerUser(MentorshipPlatform platform, Scanner scnr) {
      ...
      String[] fields = {
         ...
         "Law", "Finance", "Design", "Artificial Intelligence", "Cybersecurity",
         "Marketing", "Environmental Science", "Architecture", "Social Work",
         "Journalism", "Public Health", "Nursing", "Accounting", "Research"
      };
      for (int i = 0; i < fields.length; i++) {
         System.out.println((i + 1) + ". " + fields[i]);
      }
      System.out.print("Choice: ");
      int fieldChoice = scnr.nextInt();
      scnr.nextLine();
      String field = (fieldChoice >= 1 && fieldChoice <= fields.length)
         ? fields[fieldChoice - 1] : "Other";

      // Community selection
      System.out.println("\nSelect your community:");
      String[] communities = {
         "BIPOC", "Women in STEM", "LGBTQ+", "First-Generation", "Veterans",
         "Latina/Latino", "Asian/Pacific Islander", "Muslim/Arab",
         "Native American/Indigenous", "Disability Community",
         "International Students", "Single Parents", "Low-Income/Working Class",
         "Neurodivergent"
      };
      for (int i = 0; i < communities.length; i++) {
         System.out.println((i + 1) + ". " + communities[i]);
      }
      System.out.print("Choice: ");
      int communityChoice = scnr.nextInt();
      scnr.nextLine();
      String community = (communityChoice >= 1 && communityChoice <= communities.length)
         ? communities[communityChoice - 1] : "Other";

      // Skills selection
      System.out.println("\nSelect your skills/interests:");
      String[] allSkills = {
         "Programming", "Data Science", "Design", "Project Management",
         "Public Speaking", "Research", "Networking", "Leadership",
         "Writing", "Engineering", "Finance", "Healthcare",
         "Business", "Entrepreneurship", "Teaching", "Machine Learning",
         "Cybersecurity", "Game Development", "Counseling", "Marketing",
         "Product Management", "UX/UI Design", "Cloud Computing",
         "Public Policy", "Grant Writing", "Community Organizing"
      };
      for (int i = 0; i < allSkills.length; i++) {
         System.out.println((i + 1) + ". " + allSkills[i]);
      }
      System.out.println("Enter 3 numbers separated by Enter:");
      ArrayList<String> list = new ArrayList<String>();
      for (int i = 0; i < 3; i++) {
         System.out.print((i + 1) + ": ");
         int skillChoice = scnr.nextInt();
         scnr.nextLine();
         if (skillChoice >= 1 && skillChoice <= allSkills.length) {
            list.add(allSkills[skillChoice - 1]);
         }
      }
